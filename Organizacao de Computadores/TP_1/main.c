...
void listaInstrucoes(Instruction* instruction)
{
	for(int i = 0 ; instruction[i].opcode != -1 ; i++)
	{
		printf("INSTRUCTION[%d].opcode = %d\n", i, instruction[i].opcode);
        printf("INSTRUCTION[%d].info1 = %d\n", i, instruction[i].info1);
        printf("INSTRUCTION[%d].info2 = %d\n", i, instruction[i].info2);
        printf("INSTRUCTION[%d].info3 = %d\n", i, instruction[i].info3);
	}
}
...
int main(int argc, char**argv) {
    ...
    if (strcmp(argv[1], "random") == 0) {
		...
		ramSize = 2;
		instructions = gerarMultiplicacaoViaInstrucoes(5, 2);
	} else if (strcmp(argv[1], "div") == 0) {
		ramSize = 3;
		instructions = gerarDivisaoViaInstrucoes(100, 2);
	} else if (strcmp(argv[1], "exp") == 0) {
		ramSize = 2;
		instructions = gerarExponenciacaoViaInstrucoes(5, 5);
	}
    else {
